...
//            });
        ...
        function logoutFun() {
            $.messager
                .confirm('系统提示', '您确定要退出本次登录吗?', function (isConfirm) {
                    if (isConfirm) {
                        location.href = '${pageContext.request.contextPath }/userAction_exitAction.action';
                    }
                });
        }
        ...
        function checkForm () {
            var jQuery = $('#editPasswordForm').form("validate");
            if(jQuery){
                var v1 = $("#txtNewPass").val();
                var v2 = $("#txtRePass").val();
                if(v1==v2){

                    $.post("userAction_editPassword.action",{"password":v1},
                        function(data){
                            if(data=='1'){
                                $("#editPwdWindow").window("close");
                                alert("post");
                    ...
                    $.messager.alert('Warning','两次密码不一致');
                }
...
<div id="editPwdWindow" class="easyui-window" title="修改密码" collapsible="false" minimizable="false" modal="true"
    ...
    <div class="easyui-layout" fit="true">
        <div region="center" border="false" style="padding: 10px; background: #fff; border: 1px solid #ccc;">
            <form  id="editPasswordForm" class="">
            <table cellpadding=3>
                <tr>
                    <td>新密码：</td>
                    <td><input id="txtNewPass" type="Password" class="txt01 easyui-validatebox "
                               required="true" data-options="validType:'length[4,6]'" /></td>
        ...
        <div region="south" border="false" style="text-align: right; height: 30px; line-height: 30px;">
            <a id="btnEp" class="easyui-linkbutton" icon="icon-ok" href="javascript:void(0)" onclick="checkForm()">确定
            </a>
